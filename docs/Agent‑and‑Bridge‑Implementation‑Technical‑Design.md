...
6. 本地拨号
7. 拨号成功后回 `TrafficOpenAck(success=true)`
8. 进入数据 relay
...
`endpoint_selection_hint` 仅作为软引导。若 hint 指向的 endpoint 当前不存在、不健康、不可达或不满足本地策略，Agent 可自主选择其他健康 endpoint，无需通知 Bridge；仅当整个 service 无可用 endpoint 时，才返回 `TrafficOpenAck(success=false)`。这与“最终 endpoint 由 Agent 选择”的规则一致。
...
2. **优先尝试**在该 tunnel 上发送 `TrafficReset(traffic_id)`
...
* pre-open 失败：关闭/Reset tunnel；若 target 为 `hybrid_group`，允许 fallback

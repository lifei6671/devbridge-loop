...
- Rust 负责拉起 Go 子进程，并通过本地 `HTTP API` 或 `IPC` 与 Go 通信。
...
- [ ] 明确 Rust Host 与 Go Agent Core 的通信方式，固定为本地 `HTTP API` 或 `IPC`
- [ ] 定义 agent 与 bridge 的 tunnel 消息模型、事件类型、错误码、重连与重同步机制
...
- [ ] 实现配置加载、日志、生命周期管理
- [ ] 实现内存态状态存储，不持久化注册数据
- [ ] 提供本地管理接口：`register`、`unregister`、`discover`、`list registrations`

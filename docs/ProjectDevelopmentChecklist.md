...
- [ ] 输出系统架构图、分层图、注册时序图、入口回流时序图、出口代理时序图
- [ ] 定义 `ProtocolAdapter`、`IngressResolver`、`Forwarder`、`EgressProxy` 抽象接口
- [ ] 定义 `ServiceKey`、`InstanceKey`、`EndpointKey`、`RouteTarget`、`RouteExtractResult`
- [ ] 冻结 `instanceId` 生成规则、注册心跳协议、TTL 超时被动摘除策略
- [ ] 明确 Rust Host 与 Go Agent Core 的通信方式，固定为本地 `HTTP API`（一期不做 `IPC`）
- [ ] 定义 agent 与 bridge 的 tunnel 消息模型、事件类型、错误码、重连与重同步机制，包含 `sessionEpoch`、`resourceVersion`、`eventId`
- [ ] 定义 bridge 路由解析优先级配置模型，覆盖 `Host`、Header、`SNI`
- [ ] 定义 env 解析优先级配置模型，默认支持 `x-env` / `X-Env`
- [ ] 定义一期非功能指标（同步时延、重连恢复时间、支持注册规模、代理延迟增量）与压测口径

### P1. Tauri 桌面壳

- [ ] 创建 `Tauri + React + TypeScript + shadcn/ui` 项目骨架
- [ ] 搭建基础布局、导航与全局状态模型
- [ ] 实现主窗口、最小化到托盘、系统托盘菜单
- [ ] 实现配置目录、日志目录、基础配置加载与保存
- [ ] 实现 Rust Host 命令桥接层
- [ ] 实现 Rust Host 拉起 Go Agent Core 子进程
- [ ] 实现 Go 子进程生命周期管理、崩溃检测、自动重启
- [ ] 前端展示 Go Agent Core 运行状态

### P2. Go Agent Core 基础能力

- [ ] 创建 Go Agent Core 工程骨架
- [ ] 实现配置加载、日志、生命周期管理
- [ ] 实现内存态状态存储，不持久化注册数据
- [ ] 提供本地管理接口：`register`、`heartbeat`、`unregister`、`discover`、`list registrations`
- [ ] 实现 `LocalRegistration` 管理
- [ ] 实现 `LocalEndpoint` 管理，支持同服务多实例、多端口
- [ ] 实现注册心跳续期与 TTL 超时摘除（被动摘除）
- [ ] 实现本地状态查询接口，供 UI 和 Rust Host 获取状态
- [ ] 定义最近错误、最近请求、状态摘要等诊断输出模型

### P3. cloud-bridge 基础能力

- [ ] 创建 `cloud-bridge` Go 工程骨架
- [ ] 实现配置加载、日志、生命周期、基础管理接口
- [ ] 实现 `TunnelSession` 管理
- [ ] 实现 `ActiveIntercept` 管理
- [ ] 实现 `BridgeRoute` 生成与查询
- [ ] 实现 bridge 侧状态查询接口：session、intercept、route、错误统计

### P4. tunnel 与注册驱动同步

- [ ] 实现 agent 到 bridge 的长连接
- [ ] 实现连接建立、心跳、断线检测、自动重连
- [ ] 实现 agent 启动后的初次握手与状态上报（携带 `sessionEpoch`、快照版本）
- [ ] 实现本地注册事件同步到 bridge
- [ ] 实现本地下线事件同步到 bridge
- [ ] 实现 bridge 根据注册事件生成/更新/删除 `ActiveIntercept`
- [ ] 实现基于 `eventId` 的事件去重与幂等处理
- [ ] 实现基于 `sessionEpoch` 的旧连接事件拒绝与乱序保护
- [ ] 实现 tunnel 重连后的全量重同步
- [ ] 验证多实例、多端口下同步模型正确
- [ ] 验证重复注册、重复下线、乱序事件下状态一致性

### P5. 入口回流与路由抽象

- [ ] 实现 `RouteExtractor` 抽象与提取器注册机制
- [ ] 实现 `HostExtractor`
- [ ] 实现 `HeaderExtractor`
- [ ] 实现 `SNIExtractor`
- [ ] 实现可配置的提取优先级链路
- [ ] 实现 HTTP ingress
- [ ] 实现 gRPC ingress
- [ ] 实现 bridge 根据 `(env, serviceName, protocol)` 路由到目标 tunnel
- [ ] 实现 agent 侧本地转发，将回流请求转发到目标 localhost endpoint
- [ ] 实现统一错误分类，至少覆盖未命中路由、tunnel 离线、本地不可达、上游超时、提取失败

### P6. 出口发现与代理

- [ ] 在业务系统现有 registry 抽象中新增 `agent` 类型
- [ ] 实现 `discover` 规则：同 env 优先，未命中则 fallback 到 `base`
- [ ] 实现 HTTP 出口代理
- [ ] 实现 gRPC 出口代理
- [ ] 实现 HTTP/gRPC 协议分离的出口链路
- [ ] 实现 env 自动透传
- [ ] 实现 env 优先级配置化
- [ ] 将请求命中的 `dev/base` 结果暴露给诊断接口与 UI

### P7. Tauri UI 一期页面

- [ ] 实现 Dashboard 页面
- [ ] 展示 agent 状态、bridge 状态、tunnel 状态、当前 env、当前 rdName、接管数量、最近错误
- [ ] 实现本地服务列表页
- [ ] 展示服务名、env、instanceId、协议端口、健康状态、注册时间
- [ ] 支持刷新、手动注销、查看详情
- [ ] 实现 ActiveIntercept 列表页
- [ ] 实现日志页面
- [ ] 实现配置页面
- [ ] 页面数据全部接入 Rust Host / Go Agent Core 的真实状态接口

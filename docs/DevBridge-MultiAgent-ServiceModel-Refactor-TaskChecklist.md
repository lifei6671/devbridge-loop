...
| 2026-03-19 | Patch | Done | 已新增 Route 级 `scope_injection`（`always/missing_only/disabled`）并接入 resolver 执行顺序（RouteMatch → ScopeInjection → ScopeFallbackPolicy）；`missing_only` 明确按“namespace+environment 均非空才算完整提供”，部分提供会触发整组覆盖；HTTP/gRPC ingress 在路由完成后会按 `effective scope` 回写 `X-Bridge-Namespace/X-Bridge-Environment` 给 upstream；Route admission 已拒绝 `RouteMatch.headers` 使用保留 scope Header，自动派生 Route 已默认写入 `scope_injection=always`；并修复了“同入口条件 + 跨 scope + scope_injection 启用”未触发冲突拦截的问题（任一路由为 `always/missing_only` 时不再以 `route.scope` 作为隔离键） | 无 | 后续可在管理面补充 `scope_injection` 可视化编辑与 warning 展示（如 inject_scope 与 route.scope 不一致） |

...
func (stream *GRPCH2TunnelStream) WritePayload(ctx context.Context, payload []byte) error {
	...
	if err := stream.stream.Send(&transportgen.TunnelEnvelope{
		...
		if ctxErr := ctx.Err(); ctxErr != nil {
			// 调用方 context 取消后，当前 stream 已不可安全复用，统一收敛为关闭态。
...
func (stream *GRPCH2TunnelStream) watchCallContext(ctx context.Context) func() {
	...
	go func() {
		...
		case <-ctx.Done():
			...
			// 此时 ctx.Done() 多半来自上层 defer cancel，不应再误触发 stream 级取消。
			select {
			case <-done:
				return
			default:
			}
			// gRPC Recv/Send 不支持传入 per-call context；调用方 context 结束后
			// 必须主动 cancel 底层 stream，才能中断阻塞调用并避免 goroutine 泄漏。
			stream.cancelStream()
		case <-done:
		}
	}()

...
// SessionRole 描述 grpc_h2 session 的运行角色。
...
// SessionConfig 描述 grpc_h2 session 构造参数。
...
// NewSession 按角色创建 grpc_h2 的 transport.Session 聚合根。
func NewSession(role SessionRole, config SessionConfig) (transport.Session, error) {
	...
	if normalizedSessionID == "" {
		return nil, fmt.Errorf("new grpc session: %w: empty session id", transport.ErrInvalidArgument)
	}
	...
	case SessionRoleAgent:
		if config.TunnelProducer == nil {
			return nil, fmt.Errorf("new grpc session: %w: nil tunnel producer", transport.ErrInvalidArgument)
		}
	case SessionRoleServer:
		if config.TunnelAcceptor == nil {
			return nil, fmt.Errorf("new grpc session: %w: nil tunnel acceptor", transport.ErrInvalidArgument)
		}
		if config.TunnelPool == nil {
			return nil, fmt.Errorf("new grpc session: %w: nil tunnel pool", transport.ErrInvalidArgument)
		}
	default:
		return nil, fmt.Errorf("new grpc session: %w: unknown role=%s", transport.ErrInvalidArgument, role)
	}

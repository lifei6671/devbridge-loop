...
const (
	...
	grpcHealthService    = "user.v1.UserService"
	defaultHTTPAddr      = "127.0.0.1:18102"
	defaultGRPCAddr      = "127.0.0.1:19102"
...
func main() {
	cfg := loadConfig()
	logger := log.New(os.Stdout, "[user-service] ", log.LstdFlags|log.Lmicroseconds)
...
func newHTTPServer(cfg config, client *agentclient.Client, logger *log.Logger) *http.Server {
	...
	mux.HandleFunc("GET /api/v1/user/profile", func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(w, http.StatusOK, map[string]any{
			"serviceName": serviceName,
			"env":         cfg.EnvName,
			"instanceId":  cfg.InstanceID,
			"nickname":    "devloop-user",
			"time":        time.Now().UTC().Format(time.RFC3339),
		})
	})
	mux.HandleFunc("GET /api/v1/user/call-inventory/http", func(w http.ResponseWriter, r *http.Request) {
		requestedEnv := firstNonBlank(r.URL.Query().Get("env"), cfg.EnvName)
		baseURL := strings.TrimSpace(r.URL.Query().Get("baseUrl"))

		// 通过 agent egress 访问 inventory-service，命中 dev 时自动回流，未命中则可 base fallback。
		request := agentclient.EgressHTTPRequest{
			ServiceName: cfg.InventoryServiceName,
			Env:         requestedEnv,
			Method:      http.MethodGet,
			Path:        "/api/v1/inventory/ping",
			RawQuery:    "from=user-service",
		...
		cancel()
		if err != nil {
			logger.Printf("调用 inventory(http) 失败: %v", err)
		...
		payload := map[string]any{
			...
			"inventory":    decodeJSONOrText(response.Body),
			"egress":       response,
		}
		writeJSON(w, http.StatusOK, payload)
	})
	mux.HandleFunc("GET /api/v1/user/call-inventory/grpc", func(w http.ResponseWriter, r *http.Request) {
		requestedEnv := firstNonBlank(r.URL.Query().Get("env"), cfg.EnvName)
		baseAddress := strings.TrimSpace(r.URL.Query().Get("baseAddress"))
		healthService := firstNonBlank(r.URL.Query().Get("healthService"), "inventory.v1.InventoryService")
		...
		cancel()
		if err != nil {
			logger.Printf("调用 inventory(grpc) 失败: %v", err)
			writeJSON(w, http.StatusBadGateway, map[string]any{
				"error":        err.Error(),
				"requestedEnv": requestedEnv,
			})
...
func loadConfig() config {
	...
	return config{
		HTTPAddr:             getenv("USER_HTTP_ADDR", defaultHTTPAddr),
		GRPCAddr:             getenv("USER_GRPC_ADDR", defaultGRPCAddr),
		...
		InventoryServiceName: getenv("USER_INVENTORY_SERVICE_NAME", "inventory-service"),
	}
}
